...
profiler_cmd() {
    ...
    case "$variant" in
        v2)   echo "$INTP_ROOT/variants/v2-c-abi/intp-c-abi --pid $pid --interval $INTERVAL --duration $duration --no-prom" ;;
        v3)   echo "$INTP_ROOT/variants/v3-ebpf-ring/intp-ebpf-ring --pid $pid --interval $INTERVAL --duration $duration" ;;
